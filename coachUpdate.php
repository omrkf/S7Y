    ...
    if (!empty($_POST['UserName']) && !empty($_POST['email']) && !empty($_POST['dateofbirth'])) {
        ...
        if ($_POST['newPassword'] === $_POST['newPasswordconfirm']) {
            if ($_SESSION['password'] == $oldPassword) {
                ...
                $UPDATEch = "update coaches set  picture=?   where coach_id='{$_SESSION['coach_id']}'   ";
                
                $sql = "select a.*,ch.* from accounts a join coaches ch on ch.coach_id=a.account_id   where (account_username='$name' and password=$newPassword) Or (email='$email' and password=$newPassword) ;";
                //Prepare statement
                ...
                echo "<script>location.href='coachProfile.php?id={$_SESSION['coach_id']}'</script>";
            } else {
